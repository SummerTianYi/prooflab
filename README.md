...
After a replay action, the interface opens an evidence-driven process view instead of jumping directly to the verdict. GCN exposes seven inspectable stages: paper claim, pinned source, captured environment, measured legacy failure, Codex compatibility patch, repaired CPU run, and evidence-backed verdict. SGC exposes only the four stages supported by its retained evidence and explicitly omits unavailable timing and environment details. Each stage links to the corresponding source, log, patch, output, or manifest; the interface never fabricates timestamps or execution output for presentation.
...
- Replaying a result now opens a step-by-step process workspace. The GCN trace exposes the claim, source commit, runtime fingerprint, original failure, Codex rationale and five-file patch, repaired output, and verdict through real sealed artifacts; SGC renders the smaller trace justified by its retained evidence.
...
- Replaying SGC or GCN opens a process workspace driven by the sealed report: four justified stages for SGC and seven stages for GCN, including the measured failure, runtime, Codex patch rationale, repaired output, and verdict. Every artifact link resolves to the existing evidence bundle.
- API routes, selectable SGC/GCN UI, repair evidence display, downloadable artifact manifests, and 77 automated tests are implemented. `npm run verify` passes.
